...
import { addToCart, fetchWishlist, toggleWishlist } from "../api/cartWishlist";
...
const ProductCard = ({ product, onUnwish }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      fetchWishlist()
        .then(data => {
          const products = data?.products || data?.wishlist?.products || data?.data?.products || [];
          // Normalize both sides to string to handle id vs _id mismatch
          const ids = products.map(p => String(p._id || p.id || p));
          setWished(ids.includes(String(product._id || product.id)));
        })
        .catch(() => {});
    })();
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // addToCart in cartWishlist.js already re-fetches and emits cart-updated
      // so no need to fetch/emit here again
    } catch {}
  ...
  const handleWishlist = async () => {
    ...
    if (!token) { router.push("/login"); return; }
    try {
      await toggleWishlist(product.id || product._id, wished);
      const nowWished = !wished;
      setWished(nowWished);
      if (!nowWished && typeof onUnwish === "function") {
        onUnwish(product.id || product._id);
      }
      // toggleWishlist in cartWishlist.js already re-fetches and emits wishlist-updated
      // so no need to emit here again
    } catch {}
  };
